			...
			<div class="menu-block popup-wrap">
				...
				<div class="menu popup-block">
					...
					foreach($arResult as $arItem):?>
						...
						<?if ($arItem["IS_PARENT"]):?>
							<?//$arItem['PARAMS']['TEXT'] = 'это текст для пункта "' . $arItem['TEXT'] . '"';?>
							<?$arItem['PARAMS'] += [$arItem['TEXT'] => 'это текст для пункта "' . $arItem['TEXT'] . '"'];?>
							<?if ($arItem["DEPTH_LEVEL"] == 1):?>
								<li><a href="<?=$arItem["LINK"]?>" class="<?if ($arItem["SELECTED"]):?>color-green<?endif?>"><?=$arItem["TEXT"]?></a>
									<ul>
									<div class="menu-text"><?=$arItem['PARAMS'][$arItem['TEXT']];?></div>
							<?else:?>
								<?//$arItem['PARAMS'] += ['TEXT' => 'это текст для пункта "' . $arItem['TEXT'] . '"'];?>
